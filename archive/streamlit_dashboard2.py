...
st.mapbox_token = "pk.eyJ1Ijoic2FyYWhiZXJrIiwiYSI6ImNtaHoweGY5aDA0dWUyc3B6Z2tidDlsd2gifQ.cmzz3h6ki96G7UGQ1e_O-Q"
...
opacity = st.sidebar.slider("Transparency", 0.0, 1.0, 0.7, 0.05)
...
if lat is not None and lon is not None:
    ...
    # City mean (Air Temp or Heat Index)
    if var_choice == "Air Temperature":
        city_series = ds["air_temperature"].sel(time=slice(start_time, end_time)).mean(dim=["x","y"]) * 9/5 + 32
        loc_series = ds["air_temperature"].sel(time=slice(start_time, end_time), y=lat, x=lon, method="nearest") * 9/5 + 32
    elif var_choice == "Relative Humidity":
        city_series = ds["relative_humidity"].sel(time=slice(start_time, end_time)).mean(dim=["x","y"])
        loc_series = ds["relative_humidity"].sel(time=slice(start_time, end_time), y=lat, x=lon, method="nearest")
    else:  # Heat Index
        T = ds["air_temperature"].sel(time=slice(start_time, end_time)) * 9/5 + 32
        R = ds["relative_humidity"].sel(time=slice(start_time, end_time))
        HI = compute_heat_index(T, R)
        city_series = HI.mean(dim=["x","y"])
        loc_series = HI.sel(y=lat, x=lon, method="nearest")
    ...
    # Plot using Plotly
    fig_ts = go.Figure()
    fig_ts.add_trace(go.Scatter(x=df_ts.index, y=df_ts["City Mean"], mode="lines+markers",
                                name="City Mean", line=dict(color="blue")))
    fig_ts.add_trace(go.Scatter(x=df_ts.index, y=df_ts["Selected Location"], mode="lines+markers",
                                name="Selected Location", line=dict(color="red")))
    # Vertical line at exact selected time
    fig_ts.add_vline(x=selected_time_pd, line=dict(color="black", dash="dash"))

    fig_ts.update_layout(
        title=f"{var_choice} ±12 hours around selected time",
        xaxis_title="Time",
        yaxis_title=f"{var_choice} ({'°F' if var_choice in ['Air Temperature','Heat Index'] else '%'})",
        height=400
    )
